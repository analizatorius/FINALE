...
config = {
    "dbname": os.getenv("POSTGRES_DB", "finale"),
    "user": os.getenv("POSTGRES_USER", "finale"),
    "password": os.getenv("POSTGRES_PASSWORD", "kPWLe7X]5.NrJzTW"),
...
logger.info(f"DB config {config['dbname']} {config['user']} {config['host']} ...")
...
def pg_isready(host, user, password, dbname, port):
    while time() - start_time < check_timeout:
        try:
            ...
            logger.info(f"Postgres isn't ready. Waiting for {check_interval} sec...")
